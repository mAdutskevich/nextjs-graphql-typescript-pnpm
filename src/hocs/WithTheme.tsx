...
const withTheme = <P extends object>(WrappedComponent: React.ComponentType<P>) =>
    function WithTheme(props: P) {
        ...
        const setThemeValue = () => {
            ...
            setTheme(isDarkTheme && JSON.parse(isDarkTheme) ? darkTheme : lightTheme);
        };

        React.useLayoutEffect(() => setThemeValue(), []);

        React.useEffect(() => {
            window.addEventListener(CHANGE_THEME_EVENT, setThemeValue);

            return () => {
                window.removeEventListener(CHANGE_THEME_EVENT, setThemeValue);
            };
        }, []);
